...
def atr_series(klines, period):
    if len(klines) < period + 1:
        return [None] * len(klines)
    atrs = [None] * len(klines)
    trs = []
...
def backtest_symbol(
    ...
    log(
        f"ATR/트레일링: period={atr_period}, breakeven={breakeven_mult}xATR, trailing={trail_mult}xATR\n"
    )
    ...
    atrs = atr_series(klines, atr_period)
    ...
    for i in range(len(closes)):
        ...
        if position_side is None:
            ...
            # ATR 기반 브레이크이븐/후행 정지 갱신
            if atr_val is not None:
                if side == "LONG":
                    if trailing_stop is None and price_close - ep >= breakeven_mult * atr_val:
                        trailing_stop = ep
                        log(f"[트레일] idx={i}, LONG breakeven 활성화, stop={trailing_stop:.4f}\n")
                    if price_close - ep >= trail_mult * atr_val:
                        new_stop = price_close - trail_mult * atr_val
                        if trailing_stop is None or new_stop > trailing_stop:
                            trailing_stop = new_stop
                            log(
                                f"[트레일] idx={i}, LONG trailing 업데이트 stop={trailing_stop:.4f} (ATR {atr_val:.4f})\n"
                            )
                else:
                    if trailing_stop is None and ep - price_close >= breakeven_mult * atr_val:
                        trailing_stop = ep
                        log(f"[트레일] idx={i}, SHORT breakeven 활성화, stop={trailing_stop:.4f}\n")
                    if ep - price_close >= trail_mult * atr_val:
                        new_stop = price_close + trail_mult * atr_val
                        if trailing_stop is None or new_stop < trailing_stop:
                            trailing_stop = new_stop
                            log(
                                f"[트레일] idx={i}, SHORT trailing 업데이트 stop={trailing_stop:.4f} (ATR {atr_val:.4f})\n"
                            )
...
class BacktestGUI:
    def __init__(self, root):
        ...
        # 8행: ATR / 트레일링
        tk.Label(cfg, text="ATR 기간:", width=18, anchor="e").grid(
            row=8, column=0, padx=5, pady=3
        )
        self.trail_atr_period_entry = tk.Entry(cfg, width=8)
        self.trail_atr_period_entry.insert(0, "14")
        self.trail_atr_period_entry.grid(row=8, column=1, sticky="w")

        tk.Label(cfg, text="브레이크이븐 ATR배:", width=18, anchor="e").grid(
            row=8, column=2, padx=5, pady=3
        )
        self.breakeven_entry = tk.Entry(cfg, width=8)
        self.breakeven_entry.insert(0, "1.0")
        self.breakeven_entry.grid(row=8, column=3, sticky="w")

        tk.Label(cfg, text="트레일 ATR배:", width=18, anchor="e").grid(
            row=9, column=0, padx=5, pady=3
        )
        self.trail_entry = tk.Entry(cfg, width=8)
        self.trail_entry.insert(0, "1.5")
        self.trail_entry.grid(row=9, column=1, sticky="w")
        ...
        self.api_key_entry.grid(row=10, column=1, columnspan=3, sticky="w", padx=5)
        ...
        self.api_secret_entry.grid(row=11, column=1, columnspan=3, sticky="w", padx=5)
        ...
        chk_hedge.grid(row=14, column=2, padx=5, pady=3, sticky="w")
    ...
    def start_backtest(self):
        try:
            ...
            atr_period = int(self.trail_atr_period_entry.get().strip())
            breakeven_mult = float(self.breakeven_entry.get().strip())
            trail_mult = float(self.trail_entry.get().strip())
    ...
    def start_demo_trading(self):
        ...
        try:
            ...
            trail_atr_period = int(self.trail_atr_period_entry.get().strip())
            atr_filter_period = int(self.atr_filter_period_entry.get().strip())
    ...
    def _should_block_by_atr(self, symbol, klines):
        cfg = self.demo_cfg
        atrs = atr_series(klines, cfg["atr_filter_period"])
        last_atr = atrs[-1]
    ...
    def demo_run_symbol_once(self, symbol, long_period):
        ...
        breakeven_mult = cfg["breakeven_mult"]
        ...
        trail_atr_period = cfg["trail_atr_period"]

        # 최신 캔들 가져오기 (TESTNET)
        limit = max(long_period + 5, trail_atr_period + 50, 120)
        kl = demo_fetch_klines(symbol, interval, limit=limit)
        if len(kl) < max(long_period + 2, trail_atr_period + 2):
        ...
        if pos_side == "LONG":
            if atr_val is not None:
                if trailing_stop is None and last_price - ep >= breakeven_mult * atr_val:
                    trailing_stop = ep
                    self.append_log(
                        f"[DEMO 트레일] {symbol} LONG breakeven 활성화 stop={trailing_stop:.4f}\n"
                    )
                if last_price - ep >= trail_mult * atr_val:
                    new_stop = last_price - trail_mult * atr_val
                    if trailing_stop is None or new_stop > trailing_stop:
                        trailing_stop = new_stop
                        self.append_log(
                            f"[DEMO 트레일] {symbol} LONG stop→{trailing_stop:.4f} (ATR {atr_val:.4f})\n"
                        )
            ...
            if atr_val is not None:
                if trailing_stop is None and ep - last_price >= breakeven_mult * atr_val:
                    trailing_stop = ep
                    self.append_log(
                        f"[DEMO 트레일] {symbol} SHORT breakeven 활성화 stop={trailing_stop:.4f}\n"
                    )
                if ep - last_price >= trail_mult * atr_val:
                    new_stop = last_price + trail_mult * atr_val
                    if trailing_stop is None or new_stop < trailing_stop:
                        trailing_stop = new_stop
                        self.append_log(
                            f"[DEMO 트레일] {symbol} SHORT stop→{trailing_stop:.4f} (ATR {atr_val:.4f})\n"
                        )
            state["trailing_stop"] = trailing_stop
            tp_price = ep * (1 - tp_pct)
            sl_price = ep * (1 + sl_pct)
